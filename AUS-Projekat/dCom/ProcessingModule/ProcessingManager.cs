...
namespace ProcessingModule
{
    ...
    public class ProcessingManager : IProcessingManager
    {
        ...
        {
            /*
            PointIdentifier nivoVVode = new PointIdentifier(PointType.ANALOG_OUTPUT, 1000);
            ...
            List<PointIdentifier> list = new List<PointIdentifier> { nivoVVode, pumpa1, pumpa2, ventil, stop };
            List<IPoint> points = storage.GetPoints(list);

            if (points[4].RawValue == 0 && (pointAddress == 2002)) 
            {
                return;
            } */
        ...
        {
            double egu_value = eguConverter.ConvertToEGU(point.ConfigItem.ScaleFactor, point.ConfigItem.Deviation, newValue);
            point.EguValue = egu_value;
            point.RawValue = newValue;
            point.Timestamp = DateTime.Now;
